...
final class StorageSettingsService {
    ...
    static func fixedStoragePath(
        _ settings: [StorageSettings]
    ) -> String? {
        settings.first?.fixedStoragePath
    }

    static func fixedStorageURL(
        _ settings: [StorageSettings]
    ) throws -> URL? {
        ...
        var isStale = false
        let url = try URL.resolveSecurityScopedBookmarkData(
            bookmarkData,
            &isStale
        )
        ...
        if isStale {
            AppLogger.recordDiagnostic(
                "The fixed storage bookmark is stale.",
                "Resolve fixed storage folder",
                url
            )
        }
    ...
    static func save(
        ...
        do {
            ...
            throw AppError
                .system(
                    "The storage setting could not be saved.",
                    error.localizedDescription,
                    error
                )
        }
    ...
    static func saveFixedStorage(
        _ modelContext: ModelContext,
        _ url: URL
    ) throws {
        do {
            let bookmarkData = try URL.securityScopedBookmarkData(
                url
            )
            ...
            throw AppError
                .system(
                    "The fixed storage folder could not be saved.",
                    error.localizedDescription,
                    error
                )
        }
    ...
    static func load(
        _ modelContext: ModelContext
    ) throws -> StorageSettings {
        do {
            let settings = try fetch(
                modelContext
            )

            return settings.first ?? StorageSettings(
                .flexibility
            )
        } catch let error as AppError {
            throw error
            ...
            throw AppError
                .system(
                    "The storage setting could not be loaded.",
                    error.localizedDescription,
                    error
                )
        }
    ...
    static func reset(
        ...
        do {
            ...
            throw AppError
                .system(
                    "The storage setting could not be reset.",
                    error.localizedDescription,
                    error
                )
        }
    ...
    private static func upsert(
        _ modelContext: ModelContext,
        _ storage: StorageSettings.Storage,
        _ update: ((StorageSettings) -> Void)? = nil
    ) throws {
        let settings = try fetch(
            modelContext
        )

        let current = settings.first ?? StorageSettings(
            storage
        )
    ...
    private static func fetch(
        ...
        try modelContext
            .fetch(
                FetchDescriptor<StorageSettings>()
            )
    }
}
